...
- **普通账号登录与注册**：主站通过 `dashboard_user` 表保存用户名与 scrypt 密码哈希，注册成功后直接种 HMAC session Cookie。历史 `fs_user`/`login_challenge` 和飞书资料保留用于投手权限兼容，但飞书 OAuth、登录卡片和长连接不再作为主站登录入口。账号鉴权实现位于 `apps/web/src/lib/dashboard/account-auth.ts`，表结构由 `packages/db/src/schema.ts` 的 `ensureAuthTables` 幂等创建。
