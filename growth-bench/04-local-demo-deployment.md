...
- 首次访问 `/login` 后点击“立即注册”创建主站账号。
- 账号密码保存在本地 PostgreSQL 的 `dashboard_user` 表中。

主站已切换为普通账号密码登录/注册，飞书 OAuth 和卡片确认链路已停用。
...
外部服务凭据当前留空，包括 Athena、Facebook 和 LLM。XMP 凭据已写入 Vercel Preview/Production 的敏感环境变量。数据库 `DATABASE_URL` 和 `ATHENA_API_KEY` 等待补齐后，实时同步和业务指标接口才可用。
...
数据库结构已经初始化，且本地已验证注册、登录、退出和再次登录链路，但未导入生产数据。因此页面可以完整访问和演示交互，业务指标目前会显示为空或为 `0`。如需用于正式汇报，下一步应补齐 Vercel 的 `DATABASE_URL` / `ATHENA_API_KEY`，再导入一套脱敏样例数据。
