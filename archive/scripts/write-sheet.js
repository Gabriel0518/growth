...
const PRODUCT_ORDER = [
  'GraceChat',   // cols B-C
  'Dora And',    // cols D-E
  'Dora',        // cols F-G (displayed as "Dora iOS")
  'Doni',        // cols H-I
  'Romi',        // cols J-K (displayed as "Romi iOS")
  'Luma',        // cols L-M
  'Jovia And',   // cols N-O
  'Romi And',    // cols P-Q
];
...
async function run() {
  ...
  // Row 2 is always blank; row 3 is always the most recent data; row 4 is the diff row.
  ...
  if (readRes.code === 0 && readRes.data?.valueRange?.values?.[0]) {
    ...
    // Only use it if it looks like a data row (first cell is a time label, not "增量")
    if (r[0] && String(r[0]) !== '增量' && r[0] !== null) {
      prevRow = r;
    }
  }

  // Step 2: Insert 3 blank rows at row 2 (push everything down)
  ...
  const insertRes = await request('POST',
    `/sheets/v2/spreadsheets/${SHEET_TOKEN}/insert_dimension_range`,
    {
      dimension: {
        sheetId: SHEET_ID,
        majorDimension: 'ROWS',
        startIndex: 1,  // 0-based: insert after header (row index 1 = after row 1)
  ...
  const newRowNums = [0]; // numeric values for diff calculation

  for (const name of PRODUCT_ORDER) {
    const item = productMap[name] || {};
    const tr = parseNumber(item.totalRevenue);
    const nr = parseNumber(item.newUserRevenue);
    totalRevenue += tr;
    totalNewUser += nr;
    newRow.push(item.totalRevenue || 'N/A');
    newRow.push(item.newUserRevenue || 'N/A');
    newRowNums.push(tr);
    newRowNums.push(nr);
  }

  newRow.push('$' + totalRevenue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
  newRow.push('$' + totalNewUser.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
  newRowNums.push(totalRevenue);
  newRowNums.push(totalNewUser);

  // Step 4: Build diff row (new - previous)
  const diffRow = ['增量'];
  ...
  // Step 5: Write row 3 (new data) and row 4 (diff). Row 2 stays blank.
  // IMPORTANT: Only write to A3:S4 - the newly inserted rows.
  // Row 5+ contains previous data that was pushed down and must NOT be touched.
  ...
  // Step 6: Verify row 5 still has the previous data (integrity check)
  ...
  if (verifyRes.code === 0 && verifyRes.data?.valueRange?.values?.[0]) {
    ...
    if (prevRow && (!row5Label || String(row5Label).trim() === '')) {
      console.error('WARNING: Row 5 is empty after insert! Previous data may have been lost.');
    } else {
