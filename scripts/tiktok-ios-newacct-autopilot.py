...
# -*- coding: utf-8 -*-
"""
tiktok-ios-newacct-autopilot.py —— 新增 iOS 账户自动运维（Mora iOS）

复刻 tiktok-ios-autopilot.py 的全部逻辑（拒审上报 + eLTVROAS 阶梯调预算/关停 + 灰名单清理），
...
用法:
  python3 tiktok-ios-newacct-autopilot.py            # 真实执行
  python3 tiktok-ios-newacct-autopilot.py --dry-run  # 只算不改不发
  python3 tiktok-ios-newacct-autopilot.py --retry-budget  # 次日00:10降预算补偿
"""
...
COST_THRESHOLD = 20.0
MIN_BUDGET = 39.0
RETRY_FILE = os.path.join(WS, "output", "tt-budget-retry-ios-newacct.json")
...
def run_budget_retry():
    global TK
    items=_load_retry()
    lines=[f"🌙 TikTok 新增 iOS 账户 降预算补偿（00:10）"]
    if not items:
...
def main():
    ...
    lines=[f"🤖 TikTok 新增 iOS 账户自动运维 {ts}"+("（DRY-RUN）" if DRY else "")]
