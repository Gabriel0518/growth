...
# -*- coding: utf-8 -*-
"""
tiktok-ios-newacct-prep.py —— 新增 iOS 账户素材预备（中午11:40，对齐 tiktok-material-prep.py iOS 段）

对 Mora iOS 账户：从 TT 素材榜同名寻址（Mora 段）→ 预上传进各自 TT 创意素材库（幂等），
让晚上 23:40 build --lib-only 纯库内秒建。独立 token。

★ 补量兜底（屹恒 2026-07-22）：Mora 段同名寻址凑不满 NEED 时，从 TT 全局榜从头补，
  对【非 Mora 素材原样预上传（不改名、用别的产品素材）】补齐到 NEED——与 build 端 resolve() 补量一致，
  保证晚上纯库内建广告有足量素材。

用法:
  python3 tiktok-ios-newacct-prep.py --dry-run   # 只算库内已有/待上传清单，不真传
  python3 tiktok-ios-newacct-prep.py             # 真跑：上传入库 + 飞书
"""
...
def _upload_one(aid, nn, lib):
    """把单个素材名上传入库并取封面；成功返回 video_id 并写入 lib，失败返回 None（附因）。"""
    row=xmp_search(nn); time.sleep(6)   # XMP 20 QPM 节流
    if not (row and row.get("file_url")): return None, "noxmp"
    r2=tt_post("file/video/ad/upload",{"advertiser_id":aid,"upload_type":"UPLOAD_BY_URL",
                                       "video_url":row["file_url"],"file_name":nn})
    if r2.get("code")!=0: return None, f"upload {r2.get('code')}"
    ...
    if cover: lib[nn]=uvid; return uvid, None
    return None, "转码/封面非9:16"

def prep_product(aid, rank, seg, lib, bl):
    have=[]; uploaded=[]; failed=[]; would_upload=[]; seen=set(); skipped_bl=0; skipped_noxmp=0; topup=0
    def cur(): return len(have)+len(uploaded)+len(would_upload)
    # ── ① 主：Mora 段同名寻址 ──
    for r in rank:
        if cur() >= NEED: break
        ...
        uvid, err = _upload_one(aid, nn, lib)
        if uvid: uploaded.append(nn)
        elif err=="noxmp": skipped_noxmp+=1
        else: failed.append(f"{nn}({err})")
    # ── ② 补量兜底：不改名、原样预上传别的产品素材 ──
    if cur() < NEED:
        for r in rank:
            if cur() >= NEED: break
            raw=r["name"]
            ...
            seen.add(fn)
            if fn in lib: have.append(fn); topup+=1; continue
            if DRY: would_upload.append(fn); topup+=1; continue
            uvid, err = _upload_one(aid, fn, lib)
            if uvid: uploaded.append(fn); topup+=1
            elif err=="noxmp": skipped_noxmp+=1
            else: failed.append(f"{fn}({err})")
    return {"have":have,"uploaded":uploaded,"failed":failed,"would_upload":would_upload,
            "skipped_bl":skipped_bl,"skipped_noxmp":skipped_noxmp,"topup":topup}
...
def main():
    ...
    bl=load_blacklist()
    warn=[]
    lines=[f"🎬 TikTok 新增 iOS 账户素材预备 {d} 11:40{tag}",
           f"账户数: {len(accounts)}（Mora iOS）",
           f"素材榜窗口(TT): {dates[0]}~{dates[-1]}（3天），全局{len(rank)}条，黑名单 {len(bl)} 条"]
    ...
    for acc in accounts:
        ...
        tp = f"，含原样补量 {st['topup']}" if st['topup'] else ""
        if DRY:
            lines.append(f"\n{pname} ({aid}): TT库{len(lib)}个，命中榜 {len(st['have'])+len(st['would_upload'])} 条"
                         f"（库内已有 {len(st['have'])} / 待上传 {len(st['would_upload'])}{tp}）")
            for nn in st["would_upload"]: lines.append(f"      ⬆︎ {nn}")
        else:
            got=len(st["have"])+len(st["uploaded"])
            lines.append(f"\n{pname} ({aid}): 库内已有 {len(st['have'])} / 新上传 {len(st['uploaded'])} = {got}/{NEED} 条{tp}"
...
if __name__=="__main__":
    try: main()
        ...
        feishu(f"🛑 tiktok-ios-newacct-prep 异常中止\n{err[-400:]}"); sys.exit(1)
