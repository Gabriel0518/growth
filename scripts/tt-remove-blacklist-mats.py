#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从命中广告的 creative_list 移除黑名单素材（保留其余素材，不补新素材）。
读 /tmp/tt_bl_hits.json（由 tt-scan-blacklist-ads.py 生成）。
用法:
  source /etc/environment && python3 scripts/tt-remove-blacklist-mats.py            # DRY-RUN 只打印
  source /etc/environment && python3 scripts/tt-remove-blacklist-mats.py --apply    # 真正写回
"""
...
hits=json.load(open("/tmp/tt_bl_hits.json"))
print(f"{'[APPLY]' if APPLY else '[DRY-RUN]'} 待处理广告 {len(hits)} 条, 黑名单键={BL}\n")
...
for i,h in enumerate(hits,1):
    ...
    if not removed:
        print(f"{i:2d}. ⏭️  {name} 实际未含黑名单素材（可能已处理），跳过"); continue
    if not new_cl:
...
json.dump(results,open("/tmp/tt_bl_removed.json","w"),ensure_ascii=False,indent=2)
