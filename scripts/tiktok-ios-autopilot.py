...
# -*- coding: utf-8 -*-
"""
tiktok-ios-autopilot.py —— Romi iOS + Luma 广告自动运维（拒审上报 + eLTVROAS 阶梯调预算/关停）

用法:
  python3 tiktok-ios-autopilot.py           # 真实执行（改预算/关停/发飞书）
  python3 tiktok-ios-autopilot.py --dry-run  # 只算不改、不发（打印将要做的动作）

逻辑（屹恒 2026-07-11 定）:
  1) 读账户所有 campaign 的 ad，ad 层 secondary_status 若拒审/部分通过 → 只飞书上报，不删不停
  2) 读个人面板(syh→Romi iOS/Luma→TT→campaign)算当天 eLTVROAS
...
OPERATOR = "syh"
PRODUCTS = ["Romi iOS", "Luma", "GraceChat"]      # 面板产品名（GraceChat 2026-07-17 加入）
...
# 降预算失败（撞 TikTok「新预算≥已花费×105%」下限）时，把目标记进此文件，
# 次日 00:10 --retry-budget 模式重试（那时 cost 归零、不再撞下限）。屹恒 2026-07-20 定。
WS = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
RETRY_FILE = os.path.join(WS, "output", "tt-budget-retry-ios.json")
...
def run_budget_retry():
    ...
    items = _load_retry()
    lines = [f"🌙 TikTok iOS 降预算补偿（00:10）"]
    if not items:
...
def main():
    ...
    lines = [f"🤖 TikTok iOS 自动运维 {ts}" + ("（DRY-RUN）" if DRY else "")]
    ...
    my_camps = [c for c in all_camps
                ...
                and (c["campaign_name"].startswith("Romi iOS_") or c["campaign_name"].startswith("Luma_") or c["campaign_name"].startswith("GraceChat_"))]
    ...
    if not camp_metrics:
        lines.append("  （面板暂无 syh 的 Romi iOS/Luma TT campaign 数据，可能今天还没消耗）")
